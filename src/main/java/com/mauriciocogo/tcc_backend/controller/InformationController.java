...
public class InformationController {
    ...
    @PostMapping
    @Operation(summary = "Criar nova informação", description = "Cria uma nova informação associada a um setor específico. "
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Buscar informação por ID", description = "Retorna os detalhes de uma informação específica com base no ID fornecido.")
    @ApiResponses({
    ...
    @GetMapping("/search")
    public ResponseEntity<List<InformationResponseDTO>> searchInformations(@RequestParam("q") String keyword) {
        List<InformationResponseDTO> results = informationService.search(keyword);
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Deletar informação", description = "Remove uma informação do sistema com base no ID informado.")
    @ApiResponses({
